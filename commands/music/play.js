...
module.exports = class PlayCommand extends Command {
  ...
  async run(message, { query }) {
    ...
    if (query.match(/^(http(s)?:\/\/)?((w){3}.)?youtu(be|.be)?(\.com)?\/.+/)) {
      ...
      //    .setTitle('✅ Success')
          .setColor('#b34d49')
          .setDescription(`Succes to add **\`${video.title}\`** to queue`)   
        ...
        youtube
          ...
          .then(function(video) {
            ...
            if (message.guild.musicData.isPlaying == false) {
             ...
             // .setTitle('✅ Success')
              .setColor('#b34d49')
              .setDescription(`Succes to added ${video.title} to queue`)   
            message.say(qqew);
            }
          })
          .catch(function() {
            ...
            return message.say(
              "There is error! Please join support server or report with `todo.bug`"
            );
          });
      })
  ...
  static playSong(queue, message) {
        ...
        const dispatcher = connection
          ...
          .on('start', function() {
            ...
            const videoEmbed = new MessageEmbed()
              ...
              .setColor('#b34d49')
              .setDescription(`Playing - [${queue[0].title}](${queue[0].url}) \nWill ended in **\`${queue[0].duration}\`**`)
              //.setTimestamp()
              .setFooter(`Playing By : ${message.author.tag}`)
    ...
    //            var stopFilter = (reaction, user) => reaction.emoji.name === "⏹" && user.id === message.author.id;
        ...
        //        var stop = msg.createReactionCollector(stopFilter);
                ...
                message.channel.send("Song has been Resumed").then(async message => {
   ...
   //         })
    //    })
        next.on("collect", r => {
          ...
          msg.reactions.resolve("❎").users.remove(message.author.id)
          message.channel.send("Iam leaving now! The music queue has ended").then(async message => {
          message.delete({ timeout: 2000 });
          })
      })
        })
          ...
          .on('finish', function() {
            if (queue.length >= 1) {
              ...
              .setDescription("Music queue is ended , am leaving now, tysm to using me!")
              .setColor('#b34d49')
             message.say(embods);
              message.guild.musicData.isPlaying = false;
          ...
          .on('error', function(e) {
            message.say('Hmm, i think i cannot play the song, maybe error, pls join support server or report the bug by type `todo.bug`');
            console.error(e);
            message.guild.musicData.queue.length = 0;
            message.guild.musicData.isPlaying = false;
            message.guild.musicData.nowPlaying = null;
